...
(function(window, $) {
    ...
    wc_stripe.BaseGateway = function(params, container) {
        ...
        this.container =
            ...
            'li.payment_method_'.concat(this.gateway_id) :
            container;
    ...
    wc_stripe.BaseGateway.prototype.is_saved_method_selected = function() {
        return (
            this.is_gateway_selected() &&
            $('[name="' + this.gateway_id + '_payment_type_key"]:checked').val() ===
            'saved'
        );
    };
    /**
    ...
    wc_stripe.BaseGateway.prototype.has_checkout_error = function() {
        return (
            $('#wc_stripe_checkout_error').length > 0 && this.is_gateway_selected()
        );
    };
    /**
     * @param  {[type]}
    ...
    wc_stripe.BaseGateway.prototype.submit_error = function(error) {
        ...
        var custom_message = $(document.body).triggerHandler(
            'wc_stripe_submit_error', [message, error, this]
        );
    ...
    wc_stripe.BaseGateway.prototype.get_shipping_prefix = function() {
        if (
            this.needs_shipping() &&
            $('[name="ship_to_different_address"]').length > 0 &&
            $('[name="ship_to_different_address"]').is(':checked')
        ) {
            return 'shipping';
        }
        return 'billing';
    ...
    wc_stripe.BaseGateway.prototype.is_add_payment_method_page = function() {
        return (
            this.get_page() === 'add_payment_method' ||
            $(document.body).hasClass('woocommerce-add-payment-method')
        );
    };
    ...
    wc_stripe.BaseGateway.prototype.get_gateway_data = function() {
        var data = $(this.container)
            .find('.woocommerce_'.concat(this.gateway_id, '_gateway_data'))
            .data('gateway');
        if (typeof data === 'undefined' && this.is_current_page('checkout')) {
            ...
            if (typeof data === 'undefined') {
                data = $('.woocommerce_' + this.gateway_id + '_gateway_data').data(
                    'gateway'
                );
            }
        }
    ...
    wc_stripe.BaseGateway.prototype.get_customer_name = function(prefix) {
        return (
            this.fields.get(prefix + '_first_name') +
            ' ' +
            this.fields.get(prefix + '_last_name')
        );
    };
    /**
    ...
    wc_stripe.BaseGateway.prototype.set_payment_method = function(
        payment_method
    ) {
    ...
    wc_stripe.BaseGateway.prototype.set_selected_shipping_methods = function(
        shipping_methods
    ) {
        ...
        if (shipping_methods && $('[name^="shipping_method"]').length) {
            for (var i in shipping_methods) {
                ...
                $('[name="shipping_method[' + i + ']"][value="' + method + '"]')
                    .prop('checked', true)
                    .trigger('change');
            }
        }
        ...
        try {
            ...
            if (this.payment_request_options.requestShipping) {
                this.paymentRequest.on(
                    ...
                    this.update_shipping_address.bind(this)
                );
                this.paymentRequest.on(
                    'shippingoptionchange',
                    this.update_shipping_method.bind(this)
                );
            }

            this.paymentRequest.on(
                'paymentmethod',
                this.on_payment_method_received.bind(this)
            );
        } catch (err) {
        ...
        var options = {
            ...
            requestPayerName: function() {
                if (this.is_current_page('checkout')) {
                    ...
                    return !this.is_valid_address(
                        this.get_address_object('billing'),
                        'billing', ['email', 'phone']
                    );
                }
                return true;
            ...
            requestShipping: function() {
                if (this.needs_shipping()) {
                    ...
                    if (
                        (this.is_current_page('checkout') &&
                            !this.is_valid_address(this.get_address_object(prefix), prefix, [
                                'email',
                                'phone',
                            ])) ||
                        !this.is_current_page('checkout')
    ...
    wc_stripe.BaseGateway.prototype.on_payment_method_received = function(
        paymentResponse
    ) {
        try {
    ...
    wc_stripe.BaseGateway.prototype.update_addresses = function(data) {
        ...
        if (data.payerPhone) {
            ...
            if (
                this.fields.exists('shipping_phone') &&
                this.fields.isEmpty('shipping_phone')
            ) {
    ...
    wc_stripe.BaseGateway.prototype.populate_address_fields = function(
        address,
        prefix
    ) {
    ...
    wc_stripe.BaseGateway.prototype.populate_shipping_fields = function(
        address
    ) {
    ...
    wc_stripe.BaseGateway.prototype.get_address_object = function(prefix) {
        var address = {};
        this.get_address_fields().forEach(
            function(k) {
    ...
    wc_stripe.BaseGateway.prototype.is_valid_address = function(
        address,
        prefix,
        exclude
    ) {
        ...
        for (var i = 0; i < mappings.length; i++) {
            ...
            if (required) {
                if (!address[k] ||
                    typeof address[k] === 'undefined' ||
                    !this.fields.isValid(k, address[k], address)
                ) {
                    return false;
                }
            }
    ...
    wc_stripe.BaseGateway.prototype.process_payment = function(
        ...
        $.ajax({
                ...
                dataType: 'json',
                data: $.extend({}, this.fields.toJson(), {
                    order_id: order_id,
                    order_key: order_key,
                }),
                beforeSend: this.ajax_before_send.bind(this),
    ...
    wc_stripe.BaseGateway.prototype.handle_card_action = function(obj) {
        try {
            this.stripe
                .handleCardAction(obj.client_secret)
                .then(
                    function(result) {
                        if (result.error) {
                            this.payment_token_received = false;
                            this.submit_error(result.error);
                            this.sync_payment_intent(obj.order_id, obj.client_secret).catch(
                                function(response) {
    ...
    wc_stripe.BaseGateway.prototype.handle_card_setup = function(obj) {
        try {
            this.stripe
                .confirmCardSetup(obj.client_secret)
                .then(
                    function(result) {
                        if (result.error) {
    ...
    wc_stripe.BaseGateway.prototype.hashchange = function(e) {
        ...
        if (match) {
            try {
                ...
                if (
                    obj &&
                    obj.hasOwnProperty('client_secret') &&
     ...
     * @return {[type]}               [description]
     */
    wc_stripe.BaseGateway.prototype.sync_payment_intent = function(
        order_id,
        client_secret
    ) {
        return new Promise(
    ...
    wc_stripe.BaseGateway.prototype.serialize_fields = function() {
        return $.extend({},
            this.fields.toJson(),
            $(document.body).triggerHandler('wc_stripe_process_checkout_data', [
                this,
                this.fields,
            ])
        );
    ...
    wc_stripe.BaseGateway.prototype.map_shipping_methods = function(
        shippingData
    ) {
        var methods = {};
    ...
    wc_stripe.BaseGateway.prototype.maybe_set_ship_to_different = function() {
        ...
        if ($('[name="ship_to_different_address"]').length) {
            $('[name="ship_to_different_address"]')
                .prop(
                    'checked',
                    this.get_address_field_hash('billing') !==
                    this.get_address_field_hash('shipping')
                )
                .trigger('change');
        }
    };
                    ...
                    .done(
                        function(response) {
                            if (response.code) {
                                ...
                                this.set_selected_shipping_methods(
    ...
    wc_stripe.CheckoutGateway = function() {
        ...
        $(this.token_selector)
            .closest('form')
            .on(
                'checkout_place_order_' + this.gateway_id,
                this.checkout_place_order.bind(this)
            ); // events for showing gateway payment buttons

        $(document.body).on(
            'wc_stripe_new_method_' + this.gateway_id,
            this.on_show_new_methods.bind(this)
        );
        $(document.body).on(
            'wc_stripe_saved_method_' + this.gateway_id,
            this.on_show_saved_methods.bind(this)
        );
        $(document.body).on(
            'wc_stripe_payment_method_selected',
            this.on_payment_method_selected.bind(this)
        );

        if (this.banner_enabled()) {
            if ($('.woocommerce-billing-fields').length) {
                $('.wc-stripe-banner-checkout').css(
                    'max-width',
                    $('.woocommerce-billing-fields').outerWidth(true)
                );
            }
        }
    ...
    wc_stripe.CheckoutGateway.prototype.update_shipping_address = function() {
        return wc_stripe.BaseGateway.prototype.update_shipping_address
    ...
    wc_stripe.CheckoutGateway.prototype.on_token_received = function(
        paymentMethod
    ) {
        this.payment_token_received = true;
        this.set_nonce(paymentMethod.id);
    ...
    wc_stripe.CheckoutGateway.prototype.trigger_payment_method_selected =
        function() {
    ...
    wc_stripe.CheckoutGateway.prototype.on_payment_method_selected = function(
        e,
        payment_method
    ) {
    ...
    wc_stripe.CheckoutGateway.prototype.cart_contains_subscription = function() {
        return (
            typeof wc_stripe_cart_contains_subscription !== 'undefined' &&
            wc_stripe_cart_contains_subscription === true
        );
    };

    wc_stripe.CheckoutGateway.prototype.set_save_payment_method = function(
        bool
    ) {
        $('[name="' + this.gateway_id + '_save_source_key' + '"]').prop(
            'checked',
            bool
        );
    };
    ...
    wc_stripe.ProductGateway = function() {
        ...
        this.buttonWidth =
            $('form.cart div.quantity').outerWidth(true) +
            $('.single_add_to_cart_button').outerWidth();
        ...
        return new Promise(
            function(resolve, reject) {
                ...
                var data = {
                    ...
                    variation_id: this.is_variable_product() ?
                ...
                var fields = this.get_form()
                    .find(
                        ':not([name="add-to-cart"],[name="quantity"],[name^="attribute_"],[name="variation_id"])'
                    )
                    .serializeArray();
                if (fields) {
                    for (var i in fields) {
    ...
    wc_stripe.ProductGateway.prototype.cart_calculation = function(
        variation_id
    ) {
        return new Promise(
            function(resolve, reject) {
    ...
    wc_stripe.GooglePay.prototype.update_addresses = function(paymentData) {
        if (paymentData.paymentMethodData.info.billingAddress) {
            ...
            if (
                this.is_current_page('checkout') &&
                this.is_valid_address(this.billing_address_object, 'billing', [
                    'phone',
                    'email',
                ])
            ) {
    ...
    wc_stripe.GooglePay.prototype.update_payment_data = function(data) {
        return new Promise(
            function(resolve, reject) {
                var shipping_method =
                    data.shippingOptionData.id == 'default' ?
                    null :
                    data.shippingOptionData.id;
                $.when(
                        $.ajax({
    ...
    wc_stripe.GooglePay.prototype.on_payment_data_changed = function(address) {
        return new Promise(
            function(resolve) {
                this.update_payment_data(address)
                    .then(
                        function(response) {
    ...
    wc_stripe.GooglePay.prototype.get_payment_options = function() {
        ...
        if (this.needs_shipping()) {
            ...
            if (
                ...
                options.paymentDataCallbacks.onPaymentDataChanged =
                    this.on_payment_data_changed.bind(this);
            }
        }
    ...
    wc_stripe.GooglePay.prototype.build_payment_request = function() {
        ...
        request.allowedPaymentMethods[0].parameters.billingAddressRequired =
            function() {
                if (this.is_current_page('checkout')) {
                    var address = (this.billing_address_object =
                        this.get_address_object('billing'));
                    ...
                    if (
                        this.fields.requestFieldInWallet('billing_phone') ||
                        !this.is_valid_address(address, 'billing', ['email'])
                    ) {
                        return true;
                    }
                    return false;
    ...
    wc_stripe.GooglePay.prototype.start = function() {
        ...
        this.paymentsClient
            .loadPaymentData(this.build_payment_request())
            .then(
                ...
                function(err) {
                    ...
                    if (
                        err.statusMessage &&
                        err.statusMessage.indexOf('paymentDataRequest.callbackIntent') > -1
                    ) {
    ...
    wc_stripe.ApplePay.prototype.get_payment_request_options = function() {
        return $.extend({},
            wc_stripe.BaseGateway.prototype.get_payment_request_options.apply(
                this,
                arguments
            ), {
    ...
    wc_stripe.ApplePay.prototype.start = function(e) {
        ...
        this.paymentRequest.update(
            this.get_payment_request_update({
                total: {
                    pending: false,
                },
            })
    ...
    wc_stripe.Afterpay.prototype.is_eligible = function(price) {
        return (
            this.needs_shipping() && price > this.get_min() && price < this.get_max()
        );
    };
    ...
    wc_stripe.CheckoutFields = function(params, page) {
        ...
        this.session_values =
            ...
            sessionStorage.getItem('wc_stripe_checkout_fields') ?
            JSON.parse(sessionStorage.getItem('wc_stripe_checkout_fields')) :
            {};
        this.fields = new Map(
            Object.keys(this.params).map(
                function(k) {
        ...
        if (
            'checkout' === page ||
            ('cart' === page && $(document.body).is('.woocommerce-checkout'))
        ) {
            $(document.body).on('updated_checkout', this.updated_checkout.bind(this));
            $('form.checkout').on(
                'change',
            ...
            $('form.checkout').on(
                'change',
                '[name="ship_to_different_address"]',
                this.on_ship_to_address_change.bind(this)
            );
            this.init_i18n();
            if ($('[name="ship_to_different_address"]').is(':checked')) {
                this.update_required_fields(
    ...
    wc_stripe.CheckoutFields.prototype.update_required_fields = function(
        country,
        name
    ) {
        if (this.locales) {
            ...
            var locale =
                typeof this.locales[country] !== 'undefined' ?
    ...
    try {
        stripe = Stripe(
            wc_stripe_params_v3.api_key,
            (function() {
                if (
                    wc_stripe_params_v3.mode === 'test' &&
                    wc_stripe_params_v3.account === ''
                ) {
                    return {};
                }
    ...
    var checkoutFields = new wc_stripe.CheckoutFields(
        wc_stripe_checkout_fields,
        wc_stripe_params_v3.page
    );
})(window, jQuery);