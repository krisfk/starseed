(function($, wc_stripe) {
    ...
    CC.prototype = $.extend({},
        wc_stripe.BaseGateway.prototype,
    ...
    CC.prototype.setup_card = function() {
        if (this.is_custom_form()) {
            ...
            ['cardNumber', 'cardExpiry', 'cardCvc'].forEach(
                function(type) {
                    this[type] = this.elements.create(
                        type,
                        $.extend(true, {}, options, this.params.customFieldOptions[type])
                    );
                }.bind(this)
            );
    ...
    CC.prototype.validate_postal_field = function() {
        if ($('#billing_postcode').length && $('#stripe-postal-code').length) {
            ...
            if (this.params.postal_regex[this.fields.get('billing_country')]) {
                var regex =
                    this.params.postal_regex[this.fields.get('billing_country')],
                    postal = $('#stripe-postal-code').val(),
                    regExp = new RegExp(regex, 'i');
                if (postal !== '') {
                    if (regExp.exec(postal) !== null) {
                        $('#stripe-postal-code')
                            .addClass('StripeElement--complete')
                            .removeClass('invalid');
                    } else {
                        $('#stripe-postal-code')
                            .removeClass('StripeElement--complete')
                            .addClass('invalid');
                    }
                } else {
                    $('#stripe-postal-code')
    ...
    CC.prototype.create_card_element = function() {
        if (this.is_custom_form()) {
            if (
                $('#wc-stripe-cc-custom-form').length &&
                $('#wc-stripe-cc-custom-form').find('iframe').length == 0
            ) {
                ...
                if ($('#stripe-postal-code').length) {
                    $('#stripe-postal-code, .postalCode').on(
                        'focus',
                        function(e) {
                    ...
                    $('#stripe-postal-code').on(
                        'change',
                        this.validate_postal_field.bind(this)
                    );
    ...
    CC.prototype.place_order = function(e) {
        if (this.is_gateway_selected()) {
            if (
                ...
                this.checkout_fields_valid()
            ) {
                e.preventDefault();
                if (this.confirmedSetupIntent) {
                    return this.on_setup_intent_received(
                        this.confirmedSetupIntent.payment_method
                    );
                }
                this.stripe
                        ...
                        payment_method: {
                            ...
                            billing_details: function() {
                                ...
                                return $.extend({},
                                    ...
                                    {
                                        address: {
                                            postal_code: $('#stripe-postal-code').val(),
                                        },
                                    } :
                                    {}
                                );
    ...
    CC.prototype.checkout_place_order = function() {
        ...
        return wc_stripe.CheckoutGateway.prototype.checkout_place_order.apply(
            this,
            arguments
        );
    };
    ...
    CC.prototype.on_input_change = function(event) {
        if (event.complete) {
            var $elements = $('#wc-stripe-cc-custom-form').find(
                '.StripeElement, #stripe-postal-code'
            );
            var order = [];
            $elements.each(
                function(idx, el) {
    ...
    CC.prototype.get_billing_details = function() {
        var details =
            wc_stripe.BaseGateway.prototype.get_billing_details.call(this);
    ...
    CC.prototype.can_create_setup_intent = function() {
        return (
            ...
            this.is_change_payment_method() ||
            (this.is_current_page('checkout') &&
                this.cart_contains_subscription() &&
                this.get_gateway_data() &&
                this.get_total_price_cents() == 0) ||
            (this.is_current_page('checkout') &&
                typeof wc_stripe_preorder_exists !== 'undefined') ||
            (this.is_current_page('order_pay') &&
                'pre_order' in this.get_gateway_data() &&
                this.get_gateway_data().pre_order === true) ||
