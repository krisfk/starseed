...
if ( class_exists('PMS_Meta_Box') ){
    ...
    Class PMS_Discount_Codes_Meta_Box extends PMS_Meta_Box    {
     ...
     */
        public function output( $post ) {
            ...
            $discount = new PMS_Discount_Code( $post );

            include_once 'views/view-meta-box-discount-codes.php';
     ...
     */
     static function is_unique_code( $code ){
         ...
         if ( !empty( $post->ID ) ) {
             $discount_ID = PMS_Discount_Codes_Meta_Box::get_discount_ID_by_code($code, $post->ID);
     ...
     */
     public function save_data( $post_id ) {
         ...
         if( !empty( $_POST['pms_discount_code'] ) ) {
            ...
            $discount_code = sanitize_text_field( trim( $_POST['pms_discount_code'] ) );
         ...
         if( isset( $_POST['pms_discount_amount'] ) ) {
             ...
             $discount_amount = sanitize_text_field( trim( $_POST['pms_discount_amount'] ) );
         ...
         if( !empty( $_POST['pms_discount_subscriptions'] ) && is_array($_POST['pms_discount_subscriptions']) ){
             $discount_subscriptions = implode(',', $_POST['pms_discount_subscriptions']);
         ...
         if( isset( $_POST['pms_discount_max_uses'] ) ){
             ...
             $max_uses = trim($_POST['pms_discount_max_uses']);
         ...
         if( isset( $_POST['pms_discount_max_uses_per_user'] ) ){
             ...
             $max_uses_per_user = trim($_POST['pms_discount_max_uses_per_user']);

             if( is_numeric($max_uses_per_user) && (intval($max_uses_per_user) >= 0) && ($max_uses_per_user == round($max_uses_per_user)) )
                 update_post_meta( $post_id, 'pms_discount_max_uses_per_user', $_POST['pms_discount_max_uses_per_user'] );
             else
